...
export const QcConsoleRefined: React.FC = () => {
  ...
  return (
    <main className="p-6 pt-1 space-y-6 bg-background">
          ...
          {showFilterMenu && (
            <div className="absolute right-0 top-full mt-1 bg-white border border-outline-variant rounded shadow-lg z-50 py-1 w-40 text-xs font-semibold">
              <div 
                ...
                className={`px-md py-sm hover:bg-surface-container-low cursor-pointer ${timeFilter === 'today' ? 'text-primary font-bold' : ''}`}
              >
                Today
              </div>
              <div 
                onClick={() => { setTimeFilter('7days'); setShowFilterMenu(false); }} 
                className={`px-md py-sm hover:bg-surface-container-low cursor-pointer ${timeFilter === '7days' ? 'text-primary font-bold' : ''}`}
              >
                Last 7 Days
              </div>
              <div 
                onClick={() => { setTimeFilter('30days'); setShowFilterMenu(false); }} 
                className={`px-md py-sm hover:bg-surface-container-low cursor-pointer ${timeFilter === '30days' ? 'text-primary font-bold' : ''}`}
              >
                Last 30 Days
              </div>
            </div>
                ...
                <button 
                  ...
                  className={`w-full py-1.5 border rounded text-[11px] font-bold flex items-center justify-center gap-2 transition-all ${
                    activeAudio?.id === f.id
                      ...
                      : 'border-outline-variant text-primary hover:bg-gray-50'
                  }`}
                >
      ...
      {activeAudio && (
        <div className="bg-[#1b1c1c] text-white p-md border border-outline rounded flex items-center justify-between gap-md fixed bottom-4 right-4 max-w-sm w-full shadow-2xl z-50 animate-bounce-custom">
      ...
      {calibrationModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <form onSubmit={submitCalibration} className="bg-white border border-outline-variant max-w-sm w-full p-6 rounded shadow-xl space-y-md">
            <h3 className="font-bold text-sm uppercase border-b pb-xs border-outline-variant">
            ...
            <div className="flex gap-sm justify-end pt-sm border-t border-outline-variant">
              <button 
                ...
                className="px-md py-1.5 border border-outline-variant rounded font-bold hover:bg-gray-50 text-[11px]"
              >
                Cancel
              </button>
              <button 
                type="submit" 
                className="px-md py-1.5 bg-primary text-white rounded font-bold text-[11px] hover:brightness-110"
              >
                Book Session
              </button>
            </div>
      ...
      {evaluationModalOpen && selectedEval && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <div className="bg-white border border-outline-variant max-w-md w-full p-6 rounded shadow-xl space-y-md">
            <div className="flex justify-between items-center border-b pb-xs border-outline-variant">
            ...
            <div className="space-y-sm text-xs font-semibold">
              ...
              <div className="pt-sm border-t border-outline-variant space-y-2">
                ...
                <div className="flex justify-between p-xs bg-surface-container-low rounded">
