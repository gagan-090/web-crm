...
export const ThHomeDashboard: React.FC = () => {
  ...
  // Custom alert lists
  const alerts = [
    { id: 'campaign-sla', text: '🔥 SLA Alert: 12 Hot Campaign Leads uncalled for > 1 hour!', actionText: 'Assign & Nudge →', severity: 'red' },
    { id: 'untagged', text: '⚠ 7 calls untagged across all teams.', actionText: 'View by Team →', severity: 'red' },
    { id: 'sla', text: '⚠ 2 active SLA breaches — 1 Transporter first-call, 1 Super Premium job.', actionText: 'View →', severity: 'red' },
    { id: 'backlog', text: '37,384 uncalled leads in backlog — ₹4.23L estimated recoverable.', actionText: 'Launch Sprint →', severity: 'orange' },
    { id: 'hiring', text: '2 CRITICAL open roles unfilled beyond target hire week.', actionText: 'View Hiring →', severity: 'amber' },
  ];
  ...
  // Global activity events (Last 30 mock activities)
  const activityEvents: ActivityEvent[] = [
    ...
    { id: '5', process: 'DW', caller: 'Ankit Singh', text: 'converted DR-48190 — Verified ₹299', time: '15 min ago' },
    { id: '6', process: 'MM', caller: 'Pooja Pal', text: 'matched DR-44391 with JD-12022', time: '19 min ago' },
    { id: '7', process: 'TR', caller: 'Bhavana Tiwari', text: 'updated billing info for TR-11204', time: '22 min ago' },
    { id: '8', process: 'SC', caller: 'Akash Thakur', text: 'logged absconding status for DR-30114', time: '30 min ago' },
    { id: '9', process: 'DW', caller: 'Arpita', text: 'scheduled callback for DR-49110', time: '34 min ago' },
    { id: '10', process: 'MM', caller: 'Tanisha', text: 'confirmed placement for JD-12015', time: '41 min ago' },
  ];
  ...
  // SLA Risks
  const slaRiskItems: SlaRiskItem[] = [
    ...
    { id: 'J944', type: 'Premium Fill', process: 'MM', partyName: 'Swift Movers', timeLeft: '2.8h Left', severity: 'medium' },
    { id: 'T155', type: 'First-Call SLA', process: 'TR', partyName: 'Gati Express', timeLeft: '3.5h Left', severity: 'low' },
    { id: 'J128', type: 'Standard Fill', process: 'MM', partyName: 'Agarwal Roadlines', timeLeft: '4.1h Left', severity: 'low' },
  ];

  // Team Pulse agents list
  const agentsList = [
    ...
    { name: 'PA', fullName: 'Pallvi', role: 'DW', status: 'online' },
    { name: 'KA', fullName: 'Kajal', role: 'DW', status: 'offline' },
    { name: 'GS', fullName: 'Gagan Shukla', role: 'DW', status: 'online' },
    { name: 'AB', fullName: 'Abhi', role: 'DW', status: 'online' },

    { name: 'PP', fullName: 'Pooja Pal', role: 'MM', status: 'online' },
    { name: 'TA', fullName: 'Tanisha', role: 'MM', status: 'busy' },
    { name: 'RA', fullName: 'Raksha', role: 'MM', status: 'online' },
    { name: 'BT', fullName: 'Bhavana Tiwari', role: 'MM', status: 'online' },
    { name: 'RV', fullName: 'Ravi', role: 'MM', status: 'busy' },
    { name: 'MC', fullName: 'Minanshu', role: 'MM', status: 'online' },
    { name: 'PC', fullName: 'Pooja Chaudhary', role: 'MM', status: 'online' },
    { name: 'AT', fullName: 'Akash Thakur', role: 'SC', status: 'online' },

    { name: 'HK', fullName: 'Rahul', role: 'TL', status: 'online' },
    { name: 'DA', fullName: 'Rajendra', role: 'TL', status: 'online' },
    { name: 'TP', fullName: 'Testing Prince', role: 'TL', status: 'offline' },

    { name: 'QC', fullName: 'Quality Control', role: 'QC', status: 'online' }
  ];
      ...
      <section className="bg-white p-md rounded-sm border border-outline-variant flipkart-shadow">
        <div className="flex justify-between items-center mb-sm">
          <div>
            ...
            {(() => {
              ...
              const pctAchieved = ((482000 / adminSalesTarget) * 100).toFixed(1);
              return (
                <p className="text-lg font-extrabold text-on-surface mt-xs flex flex-wrap items-center gap-xs">
                  <span>₹4,82,000</span>
                  <span className="text-outline text-xs font-medium">of ₹{adminSalesTarget.toLocaleString()} target ({pctAchieved}% achieved) · 12 days remaining</span>
        ...
        <div className="h-6 w-full bg-surface-container rounded-sm overflow-hidden flex cursor-pointer select-none">
          <div
            ...
            style={{ width: '25.7%' }} // ₹1,24,000 / ₹4,82,000
            title="DW (Driver Welcome) — ₹1,24,000 / Target: ₹2,00,000"
          >
            DW (₹1.24L)
          </div>
          <div
            ...
            style={{ width: '45.2%' }} // ₹2,18,000 / ₹4,82,000
            title="TR (Transporter Welcome) — ₹2,18,000 / Target: ₹2,00,000"
          >
            TR (₹2.18L)
          </div>
          <div
            ...
            style={{ width: '5.8%' }} // ₹28,000 / ₹4,82,000
            title="SC (Special Categories) — ₹28,000 / Target: ₹50,000"
          >
            SC (₹28k)
          </div>
          <div
            ...
            style={{ width: '23.3%' }} // ₹1,12,000 / ₹4,82,000
            title="MM (Matchmaking) — ₹1,12,000 / Target: ₹3,50,000"
          >
            MM (₹1.12L)
          </div>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-md">
        ...
        <div className="bg-white border-t-[4px] border-t-green-500 border-x border-b border-outline-variant p-md rounded-sm flipkart-shadow flex flex-col justify-between h-[155px]">
          <div>
            ...
            <p className="font-data-mono text-sm font-extrabold mt-sm">₹1,24,000 <span className="text-outline text-[10px] font-medium">/ ₹2.00L</span></p>
            <p className="text-outline mt-xs text-[10px] font-semibold">TL: Rahul</p>
          </div>
          <div className="mt-md pt-sm border-t border-outline-variant flex justify-between items-center text-[10px]">
            <span>Calls today: <strong className="font-bold">142</strong> · Reg: <strong className="font-bold font-data-mono">{driversRegistered}</strong></span>
            <span className="text-green-600 font-bold">14.2% Conv</span>
          </div>
        ...
        <div className="bg-white border-t-[4px] border-t-orange-500 border-x border-b border-outline-variant p-md rounded-sm flipkart-shadow flex flex-col justify-between h-[155px]">
          <div>
            ...
            <p className="font-data-mono text-sm font-extrabold mt-sm">₹2,18,000 <span className="text-outline text-[10px] font-medium">/ ₹2.00L</span></p>
            <p className="text-outline mt-xs text-[10px] font-semibold">TL: Rajendra</p>
          </div>
          <div className="mt-md pt-sm border-t border-outline-variant flex justify-between items-center text-[10px]">
            <span>Calls today: <strong className="font-bold">208</strong> · Reg: <strong className="font-bold font-data-mono">{transportersRegistered}</strong></span>
            <span className="text-green-600 font-bold">18.5% Conv</span>
          </div>
        ...
        <div className="bg-white border-t-[4px] border-t-teal-500 border-x border-b border-outline-variant p-md rounded-sm flipkart-shadow flex flex-col justify-between h-[155px]">
          <div>
            ...
            <p className="font-data-mono text-sm font-extrabold mt-sm">₹28,000 <span className="text-outline text-[10px] font-medium">/ ₹50k</span></p>
            <p className="text-red-600 font-extrabold mt-xs text-[10px]">TL: Open Position</p>
          </div>
          <div className="mt-md pt-sm border-t border-outline-variant flex justify-between items-center text-[10px]">
            <span>Calls today: <strong className="font-bold">84</strong></span>
            <span className="text-red-500 font-bold">9.2% Conv</span>
          </div>
        ...
        <div className="bg-white border-t-[4px] border-t-purple-500 border-x border-b border-outline-variant p-md rounded-sm flipkart-shadow flex flex-col justify-between h-[155px]">
          <div>
            ...
            <p className="font-data-mono text-sm font-extrabold mt-sm">₹1,12,000 <span className="text-outline text-[10px] font-medium">/ ₹3.50L</span></p>
            <p className="text-outline mt-xs text-[10px] font-semibold">TL: Rajendra</p>
          </div>
          <div className="mt-md pt-sm border-t border-outline-variant flex justify-between items-center text-[10px]">
            <span>Calls today: <strong className="font-bold">116</strong> · Jobs: <strong className="font-bold font-data-mono">{postedJobsCount}</strong></span>
            <span className="text-green-600 font-bold">12.1% Conv</span>
          </div>
